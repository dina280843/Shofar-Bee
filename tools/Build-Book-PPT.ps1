...
if ($Sample) { $plan = @('title','copyright','howto','puzzle:1','write:1','puzzle:50','write:50','answers') }
else {
    ...
    switch -Regex ($spec) {
      ...
      '^copyright$' {
        $lines = @(
          ...
          'dina280843.github.io/Shofar-Bee', 'where they can still be played.', '',
          'First edition, 2026')
        Set-Box (New-Box $slide $x0 420 $cw 150) ($lines -join "`r") $FaceBody 9.5 $STONE $ppAlignLeft 14 $false $false 0 | Out-Null
      }
      '^howto$' {
      ...
      '^answers$' {
        Set-Box (New-Box $slide $x0 $MT $cw 24) 'ANSWERS' $FaceDisplay 16 $DARK $ppAlignLeft 20 $true $false 1.5 | Out-Null
        Add-Rule $slide $x0 ($MT + 26) $cw $GOLD 1.0 | Out-Null
        $gap = 14.0; $colW = ($cw - 2*$gap) / 3.0
        $blocks = @()
        foreach ($dn in 1,2) {
          $p = $data | Where-Object { $_.day -eq $dn }
          $blocks += ('DAY ' + $p.day + '  ' + $MIDDOT + '  ' + $p.hero)
          $blocks += ('centre ' + $p.center + '  pangram ' + $p.pangram + '  Queen ' + $p.queen)
          $blocks += $p.words; $blocks += ''
        }
        $per = [Math]::Ceiling($blocks.Count / 3.0)
        for ($c = 0; $c -lt 3; $c++) {
          $slice = $blocks[($c*$per)..([Math]::Min(($c+1)*$per-1, $blocks.Count-1))]
          if (-not $slice) { continue }
          Set-Box (New-Box $slide ($x0 + $c*($colW+$gap)) ($MT + 42) $colW 470) ($slice -join "`r") $FaceList 8.5 $DARK $ppAlignLeft 10.5 $false $false 0 | Out-Null
        }
      }
    }
  }
